...
export const ShelvesListPage = () => {
  ...
  const processedShelves = useMemo(() => {
    ...
    // 1. Search (ID removed from search)
    if (searchQuery) {
    ...
    if (sortConfig) {
      result.sort((a, b) => {
        ...
        // Number sorting (if needed in future)
    ...
    return (
      <div className="flex items-center justify-center h-screen bg-gray-50">
        <div className="flex flex-col items-center gap-3">
          ...
          <div className="text-gray-500 font-medium">Завантаження списку полиць...</div>
        </div>
      </div>
    );
    ...
    return (
      <div className="flex items-center justify-center h-screen bg-gray-50">
        <div className="bg-white p-6 rounded-xl shadow-lg border border-red-100 text-center">
          ...
          <p className="text-red-500">Не вдалося завантажити полиці.</p>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-gray-50/50 p-6 md:p-10">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h1 className="text-3xl font-extrabold text-slate-800 tracking-tight">Полиці</h1>
            <p className="text-slate-500 mt-1">
              Знайдено {processedShelves.length} із {shelves?.length || 0} записів
            </p>
          </div>
          ...
          {processedShelves.length > 0 ? (
              ...
              <table className="w-full whitespace-nowrap">
                <thead className="bg-slate-50 border-b border-slate-200">
                  <tr>
                    ...
                    >
                      Код полиці {sortConfig?.key === 'shelfcode' && (sortConfig.direction === 'asc' ? '↑' : '↓')}
                    </th>
                    <th 
                  ...
                  {processedShelves.map((shelf) => (
                    ...
                    >
                      <td className="px-6 py-4">
                        <span className="font-mono text-slate-700 font-semibold bg-slate-100 px-2 py-1 rounded border border-slate-200">
                          {shelf.shelfcode}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-sm text-slate-600 font-medium">
                         <span className="flex items-center gap-2">
                           ...
                           {shelf.cabinet?.name || <span className="text-slate-400 italic">Без шафи</span>}
                         </span>
                      </td>
            ...
            <div className="flex flex-col items-center justify-center py-16 text-slate-400">
              ...
              <p className="text-lg font-medium">
                {searchQuery ? 'За вашим запитом нічого не знайдено' : 'Список полиць порожній'}
              </p>
            </div>
          )}
        </div>
