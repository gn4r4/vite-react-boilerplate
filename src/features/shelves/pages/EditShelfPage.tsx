...
export const EditShelfPage = () => {
  const { shelfId } = useParams({ strict: false });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    updateShelf.mutate({
      id,
      data: payload as unknown as Partial<IShelf>,
    }, {
      onSuccess: () => navigate({ to: '/shelves' }),
      onError: () => setFormErrors("Помилка при оновленні полиці."),
    });
  };
        ...
        <div className="flex items-center gap-4 mb-8">
           ...
           <div className="flex-1 flex justify-between items-center">
             <div>
                ...
                <p className="text-slate-500">Оновлення інформації про полицю</p>
             </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden p-6 md:p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="flex gap-4 pt-4 border-t border-slate-100">
              ...
              >
                {updateShelf.isPending ? 'Збереження...' : 'Зберегти зміни'}
              </button>
            </div>
          </form>
        </div>
