...
export const GenresListPage = () => {
  ...
  const [sortConfig, setSortConfig] = useState<{ key: keyof IGenre; direction: 'asc' | 'desc' } | null>(null);

  // Обробник сортування
  const handleSort = (key: keyof IGenre) => {
    ...
    return (
      <div className="flex items-center justify-center h-screen bg-gray-50">
        <div className="flex flex-col items-center gap-3">
          ...
          <div className="text-gray-500 font-medium">Завантаження жанрів...</div>
        </div>
      </div>
    );
    ...
    return (
      <div className="flex items-center justify-center h-screen bg-gray-50">
        <div className="bg-white p-6 rounded-xl shadow-lg border border-red-100 text-center">
          ...
          <p className="text-red-500">Не вдалося завантажити жанри.</p>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-gray-50/50 p-6 md:p-10">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h1 className="text-3xl font-extrabold text-slate-800 tracking-tight">Літературні жанри</h1>
            <p className="text-slate-500 mt-1">
              Знайдено {processedGenres.length} із {genres?.length || 0} записів
            </p>
          </div>
          ...
          {processedGenres.length > 0 ? (
            <div className="overflow-x-auto">
              <table className="w-full whitespace-nowrap">
                ...
                <tbody className="divide-y divide-slate-100">
                  {processedGenres.map((g) => (
                    <tr 
                      key={g.id} 
                      className="hover:bg-blue-50/50 transition-colors group"
                    >
                      <td className="px-6 py-4 text-sm font-semibold text-slate-800 w-full">
                        {g.name}
                      </td>
                      ...
                      {!isReader && (
                        <td className="px-6 py-4 text-right whitespace-nowrap">
                          <div className="flex items-center justify-end gap-3 opacity-80 group-hover:opacity-100 transition-opacity">
                            ...
                            <button 
                              onClick={() => {
                                if (window.confirm(`Ви впевнені, що хочете видалити жанр "${g.name}"?`)) {
            ...
            <div className="flex flex-col items-center justify-center py-16 text-slate-400">
              ...
              <p className="text-lg font-medium">
                {searchQuery ? 'За вашим запитом нічого не знайдено' : 'Список жанрів порожній'}
              </p>
            </div>
          )}
        </div>
